...
const AUTOSCROLL_TIMEOUT = 200;

const NUMBER_OF_MESSAGES_USED_FOR_TITLE = 4;
...
function Hello() {
  ...
  const timeFromLastScroll = useRef(0);
  ...
  useEffect(() => {
    return window.electron.ipcRenderer.on(
      ...
      (response) => {
        ...
        const res = response as { id: string; title: string };

        if (res.title && res.id === chatId) {
          setTitle(res.title);
        }
      }
    );
  ...
  const scrollToBottom = useCallback(() => {
    timeFromLastScroll.current = Date.now();
  ...
  useEffect(() => {
    if (msgs.length && autoScrollEnabled) {
      const d = AUTOSCROLL_TIMEOUT - (Date.now() - timeFromLastScroll.current);
      const t = setTimeout(scrollToBottom, d <= 0 ? 0 : d);
  ...
  }, [msgs, autoScrollEnabled, scrollToBottom]);
  ...
  useEffect(() => {
    window.electron.ipcRenderer.on('ipc-new-chat', () => {
      console.log('Renderer: new chat');
  ...
  useEffect(() => {
    window.electron.ipcRenderer.on('ipc-focus', () => {
      console.log('Renderer: focus');
  ...
  useEffect(() => {
    if (
      msgs.length < NUMBER_OF_MESSAGES_USED_FOR_TITLE ||
      title ||
      isComputerThinking
    ) {
      return;
    ...
    window.electron.ipcRenderer.sendMessage(
      ...
      [
        ...msgs.slice(0, NUMBER_OF_MESSAGES_USED_FOR_TITLE),
        {
          role: 'user',
          content:
            'Based on the previous chat messages, generate a concise and relevant chat title, return only that title, nothing else, short as possible, no quotes. the response should title itself, nothing more, no "Chat Title: ", just title content, i should understand what this is about by the title. even if it is obviously test chat woth test or greetings messages, just create a title. if you cant generate a decent title, just reply with "New chat"',
        },
      ].map(({ role, content }) => ({
        role,
        content,
  ...
  }, [msgs, title, model, chatId, isComputerThinking]);

  return (
    <div className={styles.root} ref={focusTrapRef}>
      {windowOpened && title && <div className={styles.title}>{title}</div>}
      <div
